...
public class PetRepository implements IRepository<Pet> {
    ...
    public List<Pet> getAll() {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                ...
                SQLstr = "SELECT Genus_id, Id, Name, Birthday FROM pet_list ORDER BY Id";
    ...
    public int create(Pet pet) {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                ...
                SQLstr = "INSERT INTO pet_list (Name, Birthday, Genus_id) SELECT ?, ?, (SELECT Id FROM pet_list WHERE Genus_name = ?)";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
    ...
    public void train(int id, String command) {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                String SQLstr = "INSERT INTO pet_list (Id, Commands) SELECT ?, (SELECT Id FROM commands WHERE Command_name = ?)";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
                prepSt.setInt(1, id);
    ...
    public List<String> getCommandsById(int petId, int commands_type) {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                if (commands_type == 1) {
                    SQLstr = "SELECT Command_name FROM Commands pc JOIN commands c ON pc.CommandId = c.Id WHERE pc.PetId = ?";
                } else {
                    SQLstr = "SELECT Command_name FROM commands c JOIN Genus_command gc ON c.Id = gc.CommandId WHERE gc.Genus_id = (SELECT Genus_id FROM pet_list WHERE Id = ?)";
                }
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLstr);
                prepSt.setInt(1, petId);
                resultSet = prepSt.executeQuery();
    ...
    public static Connection getConnection() throws SQLException, IOException {
        ...
        try (FileInputStream fis = new FileInputStream(
                "D:\\!!!ХРАНИЛИЩЕ\\Programming\\GeekBrains\\Итоговая контрольная работа по блоку специализация\\Accaunt_system_nursery\\System\\src\\Resources\\database.properties")) {
...
// "D:\\!!!ХРАНИЛИЩЕ\\Programming\\GeekBrains\\Итоговая контрольная работа по
// блоку
// специализация\\Accaunt_system_nursery\\System\\src\\Resources\\database.properties"